...
int main() {
  ...
  */
  ...
  /*for split twice*/
  Pos next_pos; next_pos.x = 9; next_pos.y = 0;
  std::cout << diff(now_pos, next_pos);
  now_pos.x = 9; now_pos.y = 0;
  ...
  return 0;
  now_pos.x = 9; now_pos.y = 9;
  next_pos.x = 5;next_pos.y = 5;
  std::cout << diff(now_pos, next_pos);
  for (auto y = 5;y >= 0;--y) {
    if (y % 2 == 0) {
      for (auto x = 0;x <= 5;++x) {
