...
void make_group(std::vector<std::pair<int,int>>& number, std::vector<std::tuple<std::string,int,int>> target)
{
  for(int i = 0;i < target.size();++i){
    ...
    {
      int t,m;
      for(t = std::get<1>(target[i]);t != number[t].first ;t = number[t].first, number[t].first = number[number[t].first].first);
      for(m = std::get<2>(target[i]);m != number[m].first ;m = number[m].first, number[m].first = number[number[m].first].first);
      if(t == m)continue;
